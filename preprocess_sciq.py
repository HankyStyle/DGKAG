...
def ground(data_path, cpnet_vocab_path, pattern_path, output_path, num_processes = 1):
    ...
    for d in data:
        ...
        sentence_text = d['question']
        distractors_list = [d['distractor1'], d['distractor2'], d['distractor3']]
        answer_text = d['correct_answer']


        statement = sentence_text + ' ' + answer_text
...
def main():
    ...
    ground(input_paths['sciq']['train'], CPNET_VOCAB, PATTERN_PATH, output_paths['sciq']['grounded']['train'], args.nprocs)
    print(f'preprocess valid dataset : \n')
    ground(input_paths['sciq']['dev'], CPNET_VOCAB, PATTERN_PATH, output_paths['sciq']['grounded']['dev'], args.nprocs)
    print(f'preprocess test dataset : \n')
    ground(input_paths['sciq']['test'], CPNET_VOCAB, PATTERN_PATH, output_paths['sciq']['grounded']['test'], args.nprocs)
